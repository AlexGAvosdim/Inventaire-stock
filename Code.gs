...
* VERSION : 6.8.4 (Fix Final Dates & Robustesse)
* DATE : 26/12/2025
* ==============================================================================
*/
...
// --- SERVLET & UTILS ---
function doGet() { return HtmlService.createTemplateFromFile('Index').evaluate().setTitle('Avosdim Stock v6.8.4').addMetaTag('viewport', 'width=device-width, initial-scale=1').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL); }
...
// CORRECTIF DATE : Détection et conversion automatique ISO -> FR
function formatDateForDisplay(val) { 
  ...
  // Si c'est une chaîne ISO (contient des tirets), on tente de la convertir
...
// CORRECTIF PARSING : Supporte DD/MM/YYYY et ISO
 ...
 try {
   ...
   const items = rows.map(r => {
     ...
     return {
       ...
       // On applique le formatage intelligent ici pour l'affichage immédiat
...
function rebuildDatabase() {
 try {
   const ss = SpreadsheetApp.openById(DB_SPREADSHEET_ID);
   let backupCounts = new Map();
   let backupTags = new Map();
   ...
   history.forEach(importLog => {
     try {
       ...
       if (sheet) {
         ...
         processInventoryInMemory(virtualDb, inventory, importDateObj, importLog.name);
       }
     } catch (errFile) {}
   });
   ...
   dbSheet.getRange(1, 1, 5000, 28).setNumberFormat("@");
   const headers = ["Ref", "Nom", "Fournisseur", "Ref Fournisseur", "Rack F", "Row F", "Case F", "Surstock F", "Rack G", "Row G", "Case G", "Surstock G", "Rack A", "Row A", "Case A", "Surstock A", "Cote 1", "Cote 2", "Cote 3", "Poids", "Date Ajout", "Date Modif", "Dernière Vue (Tech)", "Date Dernier Comptage", "Temp", "Date Suppression", "Tags", "Date_Dernier_Inventaire"];
   ...
   virtualDb.forEach(item => {
     outputData.push([
       ...
       // Formatage correct lors de la reconstruction
...
function processInventoryInMemory(dbMap, newInventory, importDateObj, dateStr) {
 ...
 newInventory.forEach(item => {
   ...
   if (dbMap.has(ref)) {
     ...
     dbMap.set(ref, { ref: ref, name: item.name, dateAdd: dateStr, lastSeen: safeImportDate, tags: item.tags || "" });
   }
 });
}
...
function parseInventoryFromValues(values) {
 ...
 const colMap = { ref: col('ref'), stock: col('stock'), name: col('article/nom') > -1 ? col('article/nom') : col('nom'), suppName: col('fournisseurs/nom'), suppRef: col('fournisseurs/référence'), cote1: col('cote 1'), cote2: col('cote 2'), cote3: col('cote 3'), poids: col('poids'), rackF: rackF_idx, rowF: col('row f'), caseF: col('case f'), surF: col('surstock f'), rackG: col('rack g'), rowG: col('row g'), caseG: col('case g'), surG: col('surstock g'), rackA: col('rack a'), rowA: col('row a'), caseA: col('case a'), surA: col('surstock a'), tags: col('tags') > -1 ? col('tags') : col('étiquettes') };
 return data.map(row => {
   const getVal = (idx) => (idx > -1 && row[idx] !== undefined) ? row[idx] : "";
   const parseNum = (val) => { if (typeof val === 'string') return parseFloat(val.replace(/\s/g, '').replace(',', '.')) || 0; return (typeof val === 'number') ? val : 0; };
   const parseStr = (val) => (val ? String(val).trim() : "");
   return {
     ref: normalizeRef(parseStr(row[colMap.ref])), name: colMap.name > -1 ? parseStr(row[colMap.name]) : "N/A", qty: colMap.stock > -1 ? parseNum(row[colMap.stock]) : 0,
     suppName: parseStr(getVal(colMap.suppName)), suppRef: parseStr(getVal(colMap.suppRef)),
...
function updateMasterDbIncremental(ss, newInventory, importDateObj, importDateStr) {
 ...
 processInventoryInMemory(dbMap, newInventory, importDateObj, importDateStr);
 const outputData = [];
 dbMap.forEach(item => {
   outputData.push([
       safeString(item.ref), safeString(item.name), safeString(item.suppName), safeString(item.suppRef),
       ...
       safeNumber(item.cote1), safeNumber(item.cote2), safeNumber(item.cote3), safeNumber(item.poids),
       safeString(item.dateAdd), safeString(item.dateMod), 
       // CORRECTIF ICI : Force le format JJ/MM/AAAA lors de la sauvegarde
       formatDateForDisplay(item.lastSeen), 
...
function saveImportToHistory(csvContent, userGivenName, fileName) {
 try {
   ...
   logSheet.appendRow([sessionId, userGivenName, importDateStr, sheetName]);
   const parsedData = parseInventoryFromValues(csvData);
   let currentImportDate = parseDateLoose(userGivenName) || new Date();
   updateMasterDbIncremental(ss, parsedData, currentImportDate, userGivenName);
   SpreadsheetApp.flush();
   return { success: true, message: "Sauvegardé.", id: sessionId };
 } catch (e) { throw new Error("Erreur sauvegarde : " + e.message); }
}
...
function processComparisonFromHistory(oldSheetName, newSheetName, oldId, newId) { try { const ss = SpreadsheetApp.openById(DB_SPREADSHEET_ID); const oldSheet = ss.getSheetByName(oldSheetName); const newSheet = ss.getSheetByName(newSheetName); if (!oldSheet) throw new Error(`Onglet introuvable: "${oldSheetName}"`); if (!newSheet) throw new Error(`Onglet introuvable: "${newSheetName}"`); const oldValues = oldSheet.getDataRange().getValues(); const newValues = newSheet.getDataRange().getValues(); const oldInventory = parseInventoryFromValues(oldValues); const newInventory = parseInventoryFromValues(newValues); const comparison = compareInventoriesLogic(oldInventory, newInventory); comparison.meta = { oldName: oldId, newName: newId, date: Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "dd/MM/yyyy HH:mm") }; return cleanDataForFrontend(comparison); } catch (e) { throw new Error(e.message); } }
function cleanDataForFrontend(obj) { return JSON.parse(JSON.stringify(obj, (key, value) => { if (value && typeof value === 'object' && value.error) return null; if (typeof value === 'number' && !isFinite(value)) return 0; return value; })); }
function compareInventoriesLogic(oldInv, newInv) { const oldMap = new Map(); oldInv.forEach(i => oldMap.set(i.ref, i)); const added = []; const removed = []; const modified = []; newInv.forEach(newItem => { if (!oldMap.has(newItem.ref)) { added.push(newItem); } else { const oldItem = oldMap.get(newItem.ref); let diffs = []; if (oldItem.name !== newItem.name) diffs.push(`Nom (${oldItem.name} -> ${newItem.name})`); if (oldItem.cote1 !== newItem.cote1) diffs.push(`Cote 1 (${oldItem.cote1} -> ${newItem.cote1})`); if (oldItem.cote2 !== newItem.cote2) diffs.push(`Cote 2 (${oldItem.cote2} -> ${newItem.cote2})`); if (oldItem.cote3 !== newItem.cote3) diffs.push(`Cote 3 (${oldItem.cote3} -> ${newItem.cote3})`); if (oldItem.poids !== newItem.poids) diffs.push(`Poids (${oldItem.poids} -> ${newItem.poids})`); if (diffs.length > 0) { modified.push({ ref: newItem.ref, name: newItem.name, details: diffs }); } oldMap.delete(newItem.ref); } }); oldMap.forEach(oldItem => removed.push(oldItem)); return { kpi: { totalRefs: newInv.length, addedCount: added.length, removedCount: removed.length, modifiedCount: modified.length }, data: { added, removed, modified } }; }
...
function setupV6Architecture() {
  const ss = SpreadsheetApp.openById(DB_SPREADSHEET_ID);
  if (!ss) throw new Error("Impossible d'accéder au tableur.");
  setupSheet(ss, 'Config_Users', ['ID_User', 'Nom', 'Prenom', 'Code_PIN', 'Role', 'Zone_Defaut', 'Actif', 'Derniere_Connexion'], [['U1', 'Admin', 'Principal', '0000', 'Admin', 'All', true, '']]);
  setupSheet(ss, 'Config_Settings', ['Type', 'Cle', 'Valeur', 'Couleur_Hex', 'Description'], [['PARAM', 'SEUIL_ALERTE', '50', '', 'Seuil % écart critique'],['PARAM', 'MODE_AVEUGLE', 'true', '', 'Masquer le stock théorique (true/false)'],['TAG', 'A', '30', '#ef4444', 'Haute Rotation'],['TAG', 'B', '90', '#f97316', 'Moyenne Rotation'],['TAG', 'C', '365', '#3b82f6', 'Faible Rotation'],['TAG', 'ZOMBIE', '180', '#a855f7', 'Stock Dormant']]);
  setupSheet(ss, 'Data_CycleTasks', ['ID_Task', 'Ref_Article', 'Emplacement_Snapshot', 'Qty_Theorique_Snapshot', 'Date_Generation', 'ID_User_Assigne', 'Statut', 'Qty_Comptee', 'Date_Comptage', 'Date_Validation']);
  updateArticleStructure(ss);
}
function setupSheet(ss, n, h, d) { let s = ss.getSheetByName(n); if (!s) { s = ss.insertSheet(n); s.appendRow(h); if (d.length) d.forEach(r => s.appendRow(r)); } }
function updateArticleStructure(ss) { const s = ss.getSheetByName('BASE_ARTICLES'); if (s) { const h = s.getRange(1, 1, 1, s.getLastColumn()).getValues()[0]; if (!h.includes('Tags')) s.getRange(1, s.getLastColumn()+1).setValue('Tags'); if (!h.includes('Date_Dernier_Inventaire')) s.getRange(1, s.getLastColumn()+2).setValue('Date_Dernier_Inventaire'); } }
...
function generateTasksBatch(maxItems, zoneFilter) {
  ...
  const newTasks = selected.map(item => {
      ...
      return [Utilities.getUuid(), String(row[0]), locStr, 0, new Date(), "", "A_FAIRE", "", "", ""];
  });
  
  if(newTasks.length > 0) taskSheet.getRange(taskSheet.getLastRow()+1, 1, newTasks.length, newTasks[0].length).setValues(newTasks);
...
function getPendingValidations() {
  ...
  return pending.map(r => {
      ...
      return { taskId: r[0], ref: r[1], loc: r[2], theo: theo, counted: counted, delta: counted - theo, user: r[5], date: formatDateForDisplay(r[8]) };
  });
}
...
function saveImportToHistory(c, n, f) { 
  try {
   const ss = SpreadsheetApp.openById(DB_SPREADSHEET_ID);
   const separator = detectSeparator(c);
   const csvData = Utilities.parseCsv(c, separator);
   if (!csvData || csvData.length === 0) throw new Error("Fichier vide");
   let sheetName = n.replace(/\//g, "-");
   let sheet = ss.getSheetByName(sheetName);
   if (sheet) sheetName += "_" + Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "HHmm");
   sheet = ss.insertSheet(sheetName);
   if (csvData.length > 0) {
   ...
   const sessionId = Date.now().toString();
   const importDateStr = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "dd/MM/yyyy HH:mm:ss");
   logSheet.appendRow([sessionId, n, importDateStr, sheetName]);
   const parsedData = parseInventoryFromValues(csvData);
   let currentImportDate = parseDateLoose(n) || new Date();
   updateMasterDbIncremental(ss, parsedData, currentImportDate, n);
   SpreadsheetApp.flush();
   return { success: true, message: "Sauvegardé.", id: sessionId };
 } catch (e) { throw new Error("Erreur sauvegarde : " + e.message); }
}
